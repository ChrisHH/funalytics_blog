...
all_data %>%
  ...
  mutate(distance_meters = distm(c(reference_lon, reference_lat), c(mean_lon, mean_lat))) %>%
  ungroup() %>% 
  filter(mean_alt <= 6500) %>% 
  ggplot(aes(x = distance_meters, y = mean_alt)) +
  geom_point() +
  geom_vline(xintercept = 20000, linetype = "dashed", "gray20") +
  labs(x = "Distance of Holding to Skyline in m", y = "Mean Altitude in ft")
```
:::
