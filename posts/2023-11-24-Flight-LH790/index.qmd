...
duration <- all_data %>%
  ...
  summarise(mean_flight_duration_in_min = mean(flugdauer))

hold_duration <- duration %>% 
  filter(holding_pattern == "Hold") %>% 
  pull(mean_flight_duration_in_min)

non_hold_duration <- duration %>% 
  filter(holding_pattern == "Non-Hold") %>% 
  pull(mean_flight_duration_in_min)

hold_duration <- round(hold_duration,1)
non_hold_duration <- round(non_hold_duration,1)
...
test_result <- t.test(flugdauer ~ holding_pattern, data = duration)


```

Looks like that, when flying a holding pattern, the flight takes on average slightly longer (but the majority still below the scheduled duration). On average a flight with a holding pattern takes `r hold_duration` minutes; a flight without a holding pattern only `r non_hold_duration` minutes. A t-test reveals with a p-value of `r round(test_result$p.value,3)` that this difference of ~6 minutes is not significant.
